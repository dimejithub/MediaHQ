...
const DEMO_MEMBERS = [
  ...
  { user_id: 'demo_lead', name: 'Sarah Johnson', email: 'sarah@mediahq.com', role: 'team_lead', skills: ['ProPresenter', 'Livestream'], availability: 'available' },
...
function MemberCard({ member }) {
  const roleClass = member.role === 'admin' ? 'bg-purple-500/20 text-purple-400' :
    member.role === 'team_lead' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-700 text-slate-300';
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-slate-700 transition-all">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          ...
          <span className={`inline-block mt-1 px-2 py-0.5 text-xs rounded-full capitalize ${roleClass}`}>
