...
function MemberCard({ member, onEdit, onDelete, canEdit }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition-all relative">
      {/* 3-dot Menu (positioned absolute) */}
      {canEdit && (
        <div className="absolute top-3 right-3 z-10">
          <button 
            onClick={() => setShowMenu(!showMenu)} 
            className="p-1.5 bg-slate-800/80 hover:bg-slate-700 rounded-lg transition-all"
      ...
      <div className="px-4 pt-6 pb-4">
        ...
        <div className="flex justify-center mb-3">
          {member.avatar ? (
            <img src={member.avatar} alt={member.name} className="w-16 h-16 rounded-full border-4 border-slate-800 object-cover shadow-lg" />
          ) : (
            <div className={`w-16 h-16 rounded-full border-4 border-slate-800 ${avatarColor} flex items-center justify-center text-white text-xl font-bold shadow-lg`}>
              {initial}
            </div>
          )}
        </div>
